...
class MultiTenantMixin:
    ...
    def perform_create(self, serializer):
        """
        Injects the organization (from the user) and the requesting user 
        into the object before saving.
        """
        user = self.request.user
        ...
        # Check if the model accepts 'organization'
        if hasattr(serializer.Meta.model, 'organization'):
            # All tenant-level objects must belong to the user's organization
            if not user.organization:
                 raise PermissionDenied("User is not assigned to an organization.")
                 
            serializer.save(organization=user.organization, created_by=user)
        else:
            # For models that don't need organization or created_by (e.g., AssignmentHistory)
            serializer.save()
