...
const LOGIN_KEY = 'multiUserSessions'; // sessionStorage key to track multiple users

export default function Login() {
    const { login, isAuthenticated, role, currentUser } = useAuth();
    ...
    // Load multi-account sessions from sessionStorage
    useEffect(() => {
        const raw = sessionStorage.getItem(LOGIN_KEY);
        setLoggedInUsers(raw ? JSON.parse(raw) : []);
    }, []);

    // Handle switching between logged-in users
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const nextRole = res.role || res.user.role || 'Employee';

            // Save multi-account info in sessionStorage
            const existing = sessionStorage.getItem(LOGIN_KEY);
            const sessions = existing ? JSON.parse(existing) : [];
            const alreadyExists = sessions.find(u => u.user.email === res.user.email);

            if (!alreadyExists) {
                sessions.push({ user: res.user, access: res.access, refresh: res.refresh, role: nextRole });
                sessionStorage.setItem(LOGIN_KEY, JSON.stringify(sessions));
                setLoggedInUsers(sessions);
            }
    ...
    return (
        <div className="login-container">
            <style>{`
                .login-container { min-height:100vh; display:flex; flex-direction:column; justify-content:center; align-items:center; background-color:#f3f4f6; padding:16px; }
                .login-card { background-color:#fff; border-radius:12px; padding:32px; max-width:400px; width:100%; box-shadow:0 20px 25px -5px rgba(0,0,0,0.1),0 10px 10px -5px rgba(0,0,0,0.04); margin-bottom:16px; }
                ...
                .multi-user-list { margin-bottom:16px; width:100%; max-width:400px; }
                .multi-user-item { padding:8px 12px; border-radius:8px; background:#e5e7eb; margin-bottom:8px; display:flex; justify-content:space-between; align-items:center; cursor:pointer; }
                .multi-user-item:hover { background:#d1d5db; }
            `}</style>

            {/* Multi-Account Switcher */}
            {loggedInUsers.length > 0 && (
                <div className="multi-user-list">
                    <h4>Accounts-Logged In</h4>
                    {loggedInUsers.map((user, idx) => (
                        <div key={idx} className="multi-user-item" onClick={() => handleSwitchUser(user)}>
                            <span>{user.user.first_name} {user.user.last_name} ({user.user.email})</span>
                            <span>{user.role}</span>
                        </div>
                    ))}
                </div>
